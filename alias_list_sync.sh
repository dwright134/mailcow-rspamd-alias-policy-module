...
response=$(curl -sS -H "accept: application/json" -H "X-API-Key: $MAILCOW_API_KEY" "https://$MAILCOW_HOSTNAME/api/v1/get/alias/all") || {
  echo "$(date "+%Y-%m-%d %H:%M:%S") #2(alias-policy-sync) <$$>; log; Error: Mailcow API request failed"
  exit 1
}
# Validate response
if ! echo "$response" | jq empty 2>/dev/null && [[ ! "$response" =~ ^\[ ]]; then
  echo "$(date "+%Y-%m-%d %H:%M:%S") #2(alias-policy-sync) <$$>; log; Error: API did not return a valid JSON array"
  exit 1
fi
...
if [[ -f "$RSPAMD_POLICY_FILE" ]] && cmp -s <(echo "$json") "$RSPAMD_POLICY_FILE"; then
  echo "$(date "+%Y-%m-%d %H:%M:%S") #2(alias-policy-sync) <$$>; log; No policy changes detected. Not updating $RSPAMD_POLICY_FILE."
  exit 0
fi
...
echo "$(date "+%Y-%m-%d %H:%M:%S") #2(alias-policy-sync) <$$>; log; Policy changes detected. Updating $RSPAMD_POLICY_FILE."
