...
3. Restart the Rspamd container:
   ...
   - Write `/etc/rspamd/local.d/alias_policy.conf` with the `alias_policy {}` block and API credentials
...
The setup script writes the following configuration block to `/etc/rspamd/local.d/alias_policy.conf`:
...
| Module config | `/etc/rspamd/local.d/alias_policy.conf` | Generated `alias_policy {}` config block |
